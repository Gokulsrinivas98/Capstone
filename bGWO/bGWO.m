...
% NIter=50;
% dim=48;
disp('The BGWO algorithm is optimizing your problem...')
Alpha_score=inf;
Beta_score=inf;
Delta_score=inf;
...
NrElem=(dim+1)*2;
u=linspace(0,1,1000);
null_idx1 = u>=1/(NrElem*0.5);


%AF2       = fobj([Positions'],u);
% AF       = fobj([ones(dim,1) Positions' ones(dim,1)],u); % Linear Array Factor
...
for la=1:NIter
    AF       = fobj([Positions ones(Nagents,1)],u); % Linear Array Factor
    Gain     = 20*log10(AF(:,null_idx1));
...
% display(['The best solution obtained by bGWO is : ', num2str(Alpha_pos)]);
% display(['The best optimal value of the objective funciton found by bGWO is : ', num2str(Alpha_score)]);
